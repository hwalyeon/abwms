			...
			if (args[1] instanceof Map)
			{
				...
				if (oParam instanceof String) {
					...
					if (null != cls) {
						...
						BoundSql bSql = stmt.getBoundSql(oParam);
						List<ParameterMapping> sqlParamList = bSql.getParameterMappings();

						log.debug("params {} " ,sqlParamList);
						...
						for (int idx = 0, _max = rSqls.length ; idx < _max ; idx++ ) {
							...
							if (idx != 0) {
								...
								String fieldName = "get" + (sqlParamList.get(idx - 1).getProperty()).toLowerCase();
								List<Method> getMethods = methods.stream().filter(x -> x.getName().toLowerCase().endsWith(fieldName)).collect(Collectors.toList());

								try {
									Object params = getMethods.get(0).invoke(oParam);
									String strParam = params + "";

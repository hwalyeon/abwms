...
@Slf4j
@Service("gidxStssService")
public class GidxStssServiceImpl implements GidxStssService
{
	...
	{
		List<Map<String, Object>> result = dao.selectList("stat.hc.gidxStss.searchGidxJudgList", params);
		...
		if(result != null && result.size() > 0){
			...
			String total = null;
			for (Map<String, Object> info : result){
				newInfo.put(info.get("growJudgCd").toString(), info.get("occrCnt").toString());
				newInfoPer.put(info.get("growJudgCd").toString(), info.get("occrPer").toString());

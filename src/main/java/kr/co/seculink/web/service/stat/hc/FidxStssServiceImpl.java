...
@Slf4j
@Service("fidxStssService")
public class FidxStssServiceImpl implements FidxStssService
{
	...
	{
		List<Map<String, Object>> result = dao.selectList("stat.hc.fidxStss.searchFidxJudgList", params);
		...
		if(result != null && result.size() > 0){
			...
			String total = null;
			for (Map<String, Object> info : result){
				newInfo.put(info.get("fatJudgCd").toString(), info.get("occrCnt").toString());
				newInfoPer.put(info.get("fatJudgCd").toString(), info.get("occrPer").toString());

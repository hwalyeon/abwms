    ...
    methods:
        {
            ...
            {  
                ...
                [
                    ...
                    {name: "growJudgCd"         , index: "growJudgCd"           , label: "성장판정 코드"                  , width: 80        , align: "center"  , editable: false},
                    {name: "growJudgNm"         , index: "growJudgNm"           , label: "성장판정 명"                    , width: 80       , align: "center"   , editable: false},
                    ...
                    {name: "growActRmrk"        , index: "growActRmrk"          , label: "신체활동수준 섭취 비고"           , width: 700       , align: "center"  , editable: false},
                    ...
                    {name: "growActStndPop"     , index: "growActStndPop"       , label: "상세정보 보기"                   , width: 80       , align: "center"  ,
                        formatter: function(cellValue, options, rowObject) {
                            return '<input type="button" class="btn btn-xs btn-outline btn-success" onclick="growActStndMng.regGrowActStndPop(\'' + rowObject.growJudgCd + '\', \'' + rowObject.palCd + '\')" value="상세보기" data-toggle="modal" data-target="#growActStndDetlPopup" />';
                        }
                    }
                ];

                $("#growActStnd_list").jqGrid("GridUnload");
                $("#growActStnd_list").jqGrid($.extend(true, {}, commonEditGridOptions(),
                {
                    datatype    : "local",
                    mtype       : 'post',
                    ...
                    onPaging : function(data) {
                        onPagingCommon(data, this, function(resultMap)
                        {
                            ...
                            $this.searchGrowActStndList(false);
                        })
                    },
                    afterSaveCell : function (rowid , colId , val, e ){
                        if($("#growActStnd_list").getRowData(rowid).crud != "C" && $("#growActStnd_list").getRowData(rowid).crud != "D" ) {
                            $("#growActStnd_list").setRowData(rowid, {crud:"U"});
                        }
                    }
                }));
                resizeJqGridWidth("growActStnd_list", "growActStnd_list_wrapper");
            },
                ...
                AjaxUtil.post(
                    {
                        ...
                        {
                            saveFileLocal(response, 'growActStndMng.xls');
                        },

let ddRcmdEatStndMng = new Vue({
    ...
    methods: {
        ...
        // 영양소정보 (nutrInfoMng) 화면에서 호출 시 param 값 세팅
        setParam: function(param) {
            let $this = this;

            let params;
            if (WebUtil.isNull(param)) {
                params = WebUtil.getStorageData('window:nutrInfoMng:params', true);
            } else {
                params = param;
            }
            WebUtil.removeStorageData('window:nutrInfoMng:params');

            if (params != null && WebUtil.isNotNull(params.nutrNm)) {
                $this.params.nutrNm = params.nutrNm;
            }
        }
    },
    computed: {},
    ...
    mounted: function() {
        ...
        top.index.$on('GET_PARAM', function(params) {
            ...
            self.searchDdRcmdEatStndList(true);
        });
        self.setParam();
        self.searchDdRcmdEatStndList(true);
    }
